...
from . import mock_yin, librosa_yin
...
class TestUtils(unittest.TestCase):
    ...
    def test_mock_yin(self):
        ...
        self.eng.eval(f"""
            ...
            P = struct('sr', {fs}, 'minf0', {f0_min}, 'maxf0', {f0_max}, 'hop', {hop}, 'thresh', {thresh});
            R = yin(sig(:,1), P);
            f0 = 440*2.^(R.f0);
            ap0 = R.ap0;
        """, nargout=0)
        mat_f0 = np.squeeze(self.eng.workspace['f0'])
        mat_ap0 = np.squeeze(self.eng.workspace['ap0'])
    ...
    def test_librosa_yin(self):
        ...
        sig, fs = sf.read(audio_path)
        sig = sig[:,0]
        py_f0, py_ap0 = librosa_yin(sig, fs, f0_min, f0_max, thresh, hop)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run unit tests for the pyTMST.pyYIN module.')
